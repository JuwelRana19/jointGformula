...
## Static intervention extension

The package also supports multiple static intervention scenarios with a pooled person-time second-stage model.

Main functions:

- `run_static_intervention_meta()`
- `fit_pooled_hazard_meta_model()`
- `bootstrap_static_intervention_meta()`
- `summarize_static_intervention_bootstrap()`
- `summarize_static_scenario_risks()`

Example:

```r
scenarios <- list(
  q1 = c(NO2_RF = 4, BC_RF = 400, PM1_RF = 1.5),
  q2 = c(NO2_RF = 6, BC_RF = 550, PM1_RF = 1.8),
  q3 = c(NO2_RF = 8, BC_RF = 700, PM1_RF = 2.1),
  q4 = c(NO2_RF = 10, BC_RF = 850, PM1_RF = 2.4)
)

res_static <- run_static_intervention_meta(
  data = NO2_complete_clean,
  config = cfg,
...
summary(res_static$meta_model)
res_static$scenario_summary
```
